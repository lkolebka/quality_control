...
from _5_wricef_validation import read_wricef_tables, validate_wricef_table, print_table
from _4_approvers_validation import find_approver_table, validate_approver_table
from _6_open_point_validation import check_open_points
...
def check_document_quality(doc_path):
    ...
    for i, paragraph in enumerate(doc.paragraphs):
        if paragraph.style.name in ['Heading 1', 'Heading 2']:
            ...
            if not valid_content:
                ...
                print(f"Check 2: ❌ Following text for {paragraph.style.name} '{paragraph.text}' is too short or missing in document: {doc_path}")
            ...
            for row in table.rows:
                for cell in row.cells:
                    if "V." in cell.text:
                        ...
                        print(f"Check 3: ✅ Version number found in header of document: {doc_path} - Version: {cell.text}")
    if version_numbers:
    ...
    if not validate_approver_table(approver_table):
    ...
    if wricef_tables:
        ...
        for value, count in total_values.items():
            details.append(f"{count} IDs are '{value}'")
        ...
        if total_correct == total_rows:
            ...
            report["5. ricef_details"] = "; ".join(details)
